...
from utils import read_examples_from_file, get_labels, FeatureConverter
...
FREQ_EMBED_PATH = Path(__file__).parent / Path("utils_fine_tune/freq_embeds.npy")
...
class Segmenter(FeatureConverter):
    ...
    def __init__(self):
        ...
        tokenizer_config.freq_embed_path = FREQ_EMBED_PATH

        tokenizer = AutoTokenizer.from_pretrained(args.output_dir, **tokenizer_args, config=tokenizer_config)
        model = AutoModelForTokenClassification_Seg.from_pretrained(args.output_dir, config=tokenizer_config)
        model.to(args.device)
    ...
    def load_and_cache_examples(self, path, mode=""):
        print("load_and_cache_examples", path)
