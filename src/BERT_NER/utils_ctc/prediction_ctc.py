...
from config_ctc import parameters_ctc, BASE_CTC_DIR, FASTTEXT_BIN
...
RESOURCES = {
    "train": BASE_CTC_DIR / Path("train_freq.txt"),
    "gigaword_word": BASE_CTC_DIR / Path("gigaword_gt_2.bin"),
    "gigaword_char": BASE_CTC_DIR / Path("gigaword_char_unique.bin"),
    "stackoverflow_char": BASE_CTC_DIR / Path("no_eng_char_uniq.bin"),
    "stackoverflow_word": BASE_CTC_DIR / Path("words.bin"),
    "cs": BASE_CTC_DIR / Path("sorted_semantic_scholar_words.txt")
}
...
def populate_word_id_from_token(token, word_to_id):
    list_of_ids = []
...
def prediction_on_token_input(ctc_ip_token, ctc_model):
    ctc_tokens, ctc_features, ctc_labels = ctc_model.features.get_features_from_token(ctc_ip_token, False)

    ctc_ids = populate_word_id_from_token(ctc_ip_token, ctc_model.word_to_id)
    ...
    ctc_scores, ctc_preds = ctc_model.classify(ctc_features, ctc_x_words)
    preds = []
...
class CTCModel:
    def __init__(self, train_file, test_file):
        ...
        classifier = NeuralClassifier(len(train_features[0]), max(train_labels) + 1, vocab_size, word_embeds)
        classifier.to(device)

        optimizer = torch.optim.Adam(classifier.parameters(), lr=parameters_ctc["LR"])
        ...
        for epoch in range(parameters_ctc['epochs']):
            loss = classifier.CrossEntropy(train_features, train_x_words, train_y)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            train_scores, train_preds = classifier(train_features, train_x_words)

            test_scores, test_preds = classifier(test_features, test_x_words)
    ...
    def classify(self, ctc_features, ctc_x_words):
        return self.classifier(ctc_features, ctc_x_words)
