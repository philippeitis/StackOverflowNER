...
import tolatex
import time
from wordfreqmapper import WordFrequencyMapper
...
def create_frequency_vector():
    ...
    freq_mapper = WordFrequencyMapper(bins=parameters["freq_mapper_bin_count"], w=parameters["freq_mapper_bin_width"])
...
def prepare_train_set_dev_data():
    ...
    train_sentences = loader.load_sentences_so_w_pred(parameters["train"], parameters["train_pred"], lower, zeros,
                                                      merge_tags, set_of_selected_tags)

    if parameters["mode"] == "dev":
        dev_sentences = loader.load_sentences_so_w_pred(parameters["dev"], parameters["dev_pred"], lower, zeros,
                                                        merge_tags, set_of_selected_tags)
        test_sentences = dev_sentences
    elif parameters["mode"] == "test":
        dev_sentences = loader.load_sentences_so_w_pred(parameters["test"], parameters["test_pred"], lower, zeros,
                                                        merge_tags, set_of_selected_tags)
        test_sentences = dev_sentences
    ...
    seg_pred_embeds = np.random.uniform(-np.sqrt(0.06), np.sqrt(0.06),
                                        (parameters['segmentation_count'], parameters['segmentation_dim']))
    ctc_pred_embeds = np.random.uniform(-np.sqrt(0.06), np.sqrt(0.06),
                                        (parameters['code_recognizer_count'], parameters['code_recognizer_dim']))
...
def evaluating(model, datas, best_F, epoch_count, phase_name):
    ...
    for data in datas:
        ...
        if use_gpu:
            val, out = model(words, dwords.cuda(), sentence_seg_preds.cuda(), sentence_ctc_preds.cuda(),
                             chars2_mask.cuda(), dcaps.cuda(), chars2_length, d)
        else:
            val, out = model(words, dwords, sentence_seg_preds, sentence_ctc_preds, chars2_mask, dcaps, chars2_length,
                             d)
    ...
    if phase_name == "dev" or phase_name == "test":
        print_result.print_result(eval_result, epoch_count, parameters["sorted_entity_list_file_name"],
                                  parameters["entity_category_code"], parameters["entity_category_human_language"])
        print("-----------------------------------")
        over_all_p = eval_result['overall']['P']
        over_all_r = eval_result['overall']['R']
        over_all_f1 = eval_result['overall']['F1']
        op_line = phase_name + ": epoch: " + str(epoch_count) + " P: " + str(over_all_p) + " R: " + str(
            over_all_r) + " F1: " + str(over_all_f1) + "\n"
...
def train_model(model, step_lr_scheduler, optimizer, train_data, dev_data, test_data):
    ...
    for epoch in range(1, parameters["epochs"] + 1):
        ...
        for i, index in enumerate(np.random.permutation(len(train_data))):
            ...
            if use_gpu:
                neg_log_likelihood = model.neg_log_likelihood(sentence_tokens, sentence_in.cuda(),
                                                              sentence_seg_preds.cuda(), sentence_ctc_preds.cuda(),
                                                              targets.cuda(), chars2_mask.cuda(), caps.cuda(),
                                                              chars2_length, d)
            else:
                neg_log_likelihood = model.neg_log_likelihood(sentence_tokens, sentence_in, sentence_seg_preds,
                                                              sentence_ctc_preds, targets, chars2_mask, caps,
                                                              chars2_length, d)
