...
def print_result(eval_result, epoch_count, sorted_entity_list_file, entity_category_code,
                 entity_category_human_language):
    result = {
        "header": ["", "Precision", "Recall", "F1", "Predicted", "Correctly Predicted"],
        "rows": []
    }
    ...
    for entity in sorted_entity_list:
        ...
        l = [entity, by_category_result[entity]["P"], by_category_result[entity]["R"], by_category_result[entity]["F1"],
             by_category_result[entity]["Total Predicted"], by_category_result[entity]["Correctly Predicted"]]
        result["rows"].append(l)

    l = ["overall", over_all_result["P"], over_all_result["R"], over_all_result["F1"],
         over_all_result["Total Predicted"], over_all_result["Correctly Predicted"]]
    result["rows"].append(l)
    ...
    if not parameters['segmentation_only']:
        ...
        for entity in sorted_entity_list:
            ...
            l = [entity, by_category_result[entity]["P"], by_category_result[entity]["R"],
                 by_category_result[entity]["F1"]]
            result["rows"].append(l)
