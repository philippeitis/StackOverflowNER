...
def read_examples_from_file(path: Path, mode):
    guid_index = 1
    examples = []
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features.append(
            InputFeatures(input_ids=input_ids, input_mask=input_mask, freq_ids=word_freq_ids, md_ids=md_ids,
                          label_ids=label_ids, label_ids_ctc=label_ids_ctc, label_ids_md=label_ids_md)
        )
    return features
